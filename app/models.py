...
class User(UserMixin, db.Model):
    ...
    # определение ассоциаций с подписчиками:
    ...
    # метод проверки наличия подписки пользователя для исключения дублирования подписок
    def is_following(self, user):
        query = self.following.select().where(User.id == user.id)
        return db.session.scalar(query) is not None

    # метод вывода счетчика подписчиков
    def followers_count(self):
        query = sa.select(sa.func.count()).select_from(self.followers.select().subquery())
        return db.session.scalar(query)

    # метод вывода счетчика подписок
    def following_count(self):
        query = sa.select(sa.func.count()).select_from(self.following.select().subquery())
        return db.session.scalar(query)

    # запрос постов подписок
...
class Post(db.Model):
    ...
    def __repr__(self):
        return f'<Post: {self.body}, time: {self.timestamp}, user_id: {self.user_id}>'
